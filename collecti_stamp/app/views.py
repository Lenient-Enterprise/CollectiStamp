...
def all_tickets(request):
    ...
    return render(request, 'collecti_stamp/all.html', {'collecti_stamp': tickets})
...
def tickets_details(request, ticket_id):
    ...
    return render(request, 'collecti_stamp/details.html', {'ticket': ticket, 'qrcode': qrcode, 'users': users})
...
def use_ticket(request, ticket_id):
    ...
    if fecha_anterior < fecha_actual <= ticket.finish_date:
        return redirect('collecti_stamp/details.html',
                        {'ticket': ticket, 'message': 'Ticket was successfully used', 'status': 'Success'})
...
def create_tickets(request):
    if request.method == 'POST':
        ...
        num_tickets = int(request.POST.get('num_tickets', 0))  # Obtener el número de collecti_stamp del request
        ...
        form = TicketsForm()
        return render(request, 'collecti_stamp/create.html', {'form': form})
